...
namespace TinyGiantStudio.BetterInspector
{
    ...
    ///
    /// Note to self:
    /// 1. Should copy property path copy something different if workspace is global?
    /// 2. Maybe add Enable/Disable field rounding?
    /// 3. Maybe add texture icon instead of the handles for _showSizeGizmosLabelHandle
    /// 4. Add setting options to context menus
    ///
    /// Maybe:
    ...
    public class BetterTransformEditor : Editor
    {
        ...
        private void SetupMainControls()
        {
            ...
            toolbarGroupBox = root.Q<GroupBox>("ToolbarsGroupBox");
        ...
        private void UpdatePasteButtons()
        {
            ...
            if (targets.Length == 1)
            {
                ...
                if (exists)
                {
                    ...
                    pastePositionButton.tooltip = "Paste " + x + "," + y + "," + z + " to " + editorSettings.CurrentWorkSpace + " position.";
                    pasteRotationButton.tooltip = "Paste " + x + "," + y + "," + z + " to " + editorSettings.CurrentWorkSpace + " rotation.";
                    pasteScaleButton.tooltip = "Paste " + x + "," + y + "," + z + " to " + editorSettings.CurrentWorkSpace + " scale.";
                }
            }
            else
            {
        ...
        private void SetupWorkSpace()
        {
            ...
            if (editorSettings.showSiblingIndex)
        ...
        private void UpdatePosition_buttons()
        {
            ...
            {
                copyPositionButton.style.display = DisplayStyle.Flex;
                pastePositionButton.style.display = DisplayStyle.Flex;
                resetPositionButton.style.display = DisplayStyle.Flex;

                copyPositionButton.tooltip = "Copy " + editorSettings.CurrentWorkSpace + " position.";
                pastePositionButton.tooltip = "Paste to " + editorSettings.CurrentWorkSpace + " position.";
                resetPositionButton.tooltip = "Reset " + editorSettings.CurrentWorkSpace + " position to zero.";
            }
            else
            {
                copyPositionButton.style.display = DisplayStyle.None;
                pastePositionButton.style.display = DisplayStyle.None;
                resetPositionButton.style.display = DisplayStyle.None;
            }
        }
        ...
        private void CopyPosition()
        {
            ...
            {
                if (editorSettings.CurrentWorkSpace == BetterTransformSettings.WorkSpace.World)
                    EditorGUIUtility.systemCopyBuffer = "Vector3" + transform.position.ToString();
                else
                    EditorGUIUtility.systemCopyBuffer = "Vector3" + transform.localPosition.ToString();
        ...
        private void UpdateRotation_label()
        {
            ...
            else
            {
                rotationLabel.tooltip = "The world rotation of this GameObject.\n\n" +
                    ...
                    "That's why it is retrieved from the quaternion rotation of the transform and although it is effectively the value you set, it looks different.";
        ...
        private void CopyRotationEulerAngles()
        {
            ...
            {
                if (editorSettings.CurrentWorkSpace == BetterTransformSettings.WorkSpace.World)
                    EditorGUIUtility.systemCopyBuffer = "Vector3" + transform.eulerAngles.ToString();
                else
                    EditorGUIUtility.systemCopyBuffer = "Vector3" + transform.localEulerAngles.ToString(); //This used to be :myTarget.localRotation.eulerAngles.ToString(). Is there any difference?
            }
            else
            {
        ...
        {
            if (editorSettings.CurrentWorkSpace == BetterTransformSettings.WorkSpace.World)
                EditorGUIUtility.systemCopyBuffer = "Quaternion" + transform.rotation.ToString();
            else
                EditorGUIUtility.systemCopyBuffer = "Quaternion" + transform.localRotation.ToString();
        ...
        private void UpdateRotation_buttons()
        {
            if (editorSettings.ShowCopyPasteButtons)
            {
                copyRotationButton.style.display = DisplayStyle.Flex;
                pasteRotationButton.style.display = DisplayStyle.Flex;
                resetRotationButton.style.display = DisplayStyle.Flex;

                copyRotationButton.tooltip = "Copy " + editorSettings.CurrentWorkSpace + " Euler Angles rotation";
                pasteRotationButton.tooltip = "Paste to " + editorSettings.CurrentWorkSpace + " Euler Angles rotation";
                resetRotationButton.tooltip = "Reset " + editorSettings.CurrentWorkSpace + " Euler Angles rotation to zero.";
            }
            else
            {
                copyRotationButton.style.display = DisplayStyle.None;
                pasteRotationButton.style.display = DisplayStyle.None;
                resetRotationButton.style.display = DisplayStyle.None;
            }
        }
        ...
        private void UpdateScale_buttons()
        {
            if (editorSettings.ShowCopyPasteButtons)
            {
                ...
                copyScaleButton.tooltip = "Copy " + editorSettings.CurrentWorkSpace + " scale.";
                pasteScaleButton.tooltip = "Paste to " + editorSettings.CurrentWorkSpace + " scale.";
                resetScaleButton.tooltip = "Reset " + editorSettings.CurrentWorkSpace + " scale to one.";
                if (editorSettings.CurrentWorkSpace == BetterTransformSettings.WorkSpace.World)
        ...
        private void CopyScale()
        {
            ...
            {
                if (editorSettings.CurrentWorkSpace == BetterTransformSettings.WorkSpace.World)
                    EditorGUIUtility.systemCopyBuffer = "Vector3" + transform.lossyScale.ToString();
                else
                    EditorGUIUtility.systemCopyBuffer = "Vector3" + transform.localScale.ToString();
            }
            else
            {
        ...
        private GroupBox allVariablesToolbarHolder;

        private void SetupCommonButtons()
        {
            allVariablesToolbarHolder = root.Q<GroupBox>("AllVariablesToolbarHolder");
            UpdateCommonButtonVisibility();

            var resetAllButton = allVariablesToolbarHolder.Q<ToolbarButton>("Reset");
        ...
        {
            if (editorSettings.ShowCopyPasteButtons && editorSettings.ShowAllVariableCopyPasteButtons)
                allVariablesToolbarHolder.style.display = DisplayStyle.Flex;
            else
                allVariablesToolbarHolder.style.display = DisplayStyle.None;
        }
        ...
        private void SetupSizeCommon()
        {
            ...
            sizeCenterFoldoutField = sizeFoldout.Q<Vector3Field>("CenterFoldoutField");
        }
        ...
        private void SetupSize(CustomFoldoutSetup customFoldoutSetup)
        {
            ...
            sizeCenterFoldoutField.SetEnabled(false);
            ...
            if (editorSettings.ShowSizeCenter)
                sizeCenterFoldoutField.style.display = DisplayStyle.Flex;
            else
                sizeCenterFoldoutField.style.display = DisplayStyle.None;

            manualUpdateButton = root.Q<Button>("CheckSizeButton");
            if (manualUpdateButton != null) manualUpdateButton.style.display = DisplayStyle.None;
        ...
        private void SetupInLineSizeView()
        {
            ...
            {
                inlineSizeGroupBox.Add(sizeFoldoutField.parent);
        ...
        private void UpdateInLineSizeView()
        {
            ...
            else //show size in foldout
            {
                ...
                content.Insert(0, sizeFoldoutField.parent);

                GroupBox InlineSizeButtonsGroupBox = root.Q<GroupBox>("InlineSizeButtonsGroupBox");

                GroupBox headerRightSide = sizeFoldout.Q<GroupBox>("Header").Q<GroupBox>("RightSide");
                headerRightSide.Add(InlineSizeButtonsGroupBox.Q<GroupBox>("GizmoGroupBox"));
                headerRightSide.Add(InlineSizeButtonsGroupBox.Q<GroupBox>("SizeTypeGroupBox"));
                headerRightSide.Add(InlineSizeButtonsGroupBox.Q<GroupBox>("CalculationTypeGroupBox"));
                headerRightSide.Add(unitDropDownField);
            }
        }
        ...
        private Bounds GetSizeWithChildren(Transform target, bool showWarningIfTooManyChild = false)
        {
            if (editorSettings.CurrentSizeType == BetterTransformSettings.SizeType.Renderer)
            {
                ...
                return CheckFilterBounds(target, showWarningIfTooManyChild);
        }
        ...
        private Bounds CheckFilterBounds(Transform target, bool showWarningIfTooManyChild = false)
        {
            bool firstRenderer = true;
            ...
            if (meshFilters.Length != 0)
            {
                for (int i = 0; i < meshFilters.Length; ++i)
                {
                    ...
                    for (int j = 0; j < vertices.Length; ++j)
                    {
                        ...
                        bounds.Encapsulate(transform.InverseTransformPoint(meshTransform.TransformPoint(v)));
                    }
                }
            }
            //else
        ...
        private void SetupSettingsFoldouts()
        {
            ...
            customFoldoutSetup.SetupFoldout(settingsFoldout.Q<GroupBox>("PrefabNotesGroupBox"));
        ...
        private void ToggleSettings(CustomFoldoutSetup customFoldoutSetup, GroupBox settings, Toggle foldoutToggle, GroupBox content, bool target)
        {
            customFoldoutSetup.SwitchContent(content, target);
        ...
        private void SetupSettingsFields()
        {
            ...
            Button scaleSettingsButton = root.Q<Button>("ScaleSettingsButton");
        ...
        private void UpdateSizeLabelSettings(GroupBox settings)
        {
            GroupBox sizeFoldoutContent = settings.Q<GroupBox>("SizeGizmoLabelFoldoutContent");

            if (editorSettings.ShowSizeGizmoLabel)
                sizeFoldoutContent.style.display = DisplayStyle.Flex;
            else
                sizeFoldoutContent.style.display = DisplayStyle.None;
        }
        ...
        private void SetupViewWidthAdaptionForSize()
        {
            if (targets.Length == 1)
            {
                if (editorSettings.ShowSizeFoldout || editorSettings.ShowSizeInLine)
                {
                    ...
                    sizeLabel = sizeFoldoutField.parent.Q<Label>("SizeLabel");
        ...
        {
            if (targets.Length == 1 && (editorSettings.ShowSizeFoldout || editorSettings.ShowSizeInLine))
            {
                ...
                if (refreshLabel != null)
                    refreshLabel.style.display = width > 415 ? DisplayStyle.Flex : DisplayStyle.None;
                ...
                {
                    gizmoOnLabel.style.display = width > 340 ? DisplayStyle.Flex : DisplayStyle.None;
                    gizmoOffLabel.style.display = width > 340 ? DisplayStyle.Flex : DisplayStyle.None;
                }

                if (editorSettings.ShowSizeFoldout)
                {
                    rendererSizeTypeLabel.style.display = width > 345 ? DisplayStyle.Flex : DisplayStyle.None;
                    meshSizeTypeLabel.style.display = width > 345 ? DisplayStyle.Flex : DisplayStyle.None;
                }
                else
                {
                ...
                hierarchyLabel.style.display = width > 300 ? DisplayStyle.Flex : DisplayStyle.None;
                selfSizeLabel.style.display = width > 300 ? DisplayStyle.Flex : DisplayStyle.None;
                ...
                if (width > 265)
                {
                    refreshSizeButton.AddToClassList("toolbarComboButton-normal");
                    refreshSizeButton.RemoveFromClassList("toolbarComboButton-shortened");

                    gizmoOffButton.AddToClassList("toolbarComboButton-normal");
                    gizmoOnButton.RemoveFromClassList("toolbarComboButton-shortened");

                    gizmoOnButton.AddToClassList("toolbarComboButton-normal");
                    gizmoOffButton.RemoveFromClassList("toolbarComboButton-shortened");

                    rendererSizeButton.AddToClassList("toolbarComboButton-normal");
                    rendererSizeButton.RemoveFromClassList("toolbarComboButton-shortened");

                    filterSizeButton.AddToClassList("toolbarComboButton-normal");
                    filterSizeButton.RemoveFromClassList("toolbarComboButton-shortened");

                    hierarchySizeButton.AddToClassList("toolbarComboButton-normal");
                    hierarchySizeButton.RemoveFromClassList("toolbarComboButton-shortened");

                    selfSizeButton.AddToClassList("toolbarComboButton-normal");
                    selfSizeButton.RemoveFromClassList("toolbarComboButton-shortened");
                }
                else
                {
                    refreshSizeButton.RemoveFromClassList("toolbarComboButton-normal");
                    refreshSizeButton.AddToClassList("toolbarComboButton-shortened");

                    gizmoOffButton.RemoveFromClassList("toolbarComboButton-normal");
                    gizmoOffButton.AddToClassList("toolbarComboButton-shortened");

                    gizmoOnButton.RemoveFromClassList("toolbarComboButton-normal");
                    gizmoOnButton.AddToClassList("toolbarComboButton-shortened");

                    rendererSizeButton.RemoveFromClassList("toolbarComboButton-normal");
                    rendererSizeButton.AddToClassList("toolbarComboButton-shortened");

                    filterSizeButton.RemoveFromClassList("toolbarComboButton-normal");
                    filterSizeButton.AddToClassList("toolbarComboButton-shortened");

                    hierarchySizeButton.RemoveFromClassList("toolbarComboButton-normal");
                    hierarchySizeButton.AddToClassList("toolbarComboButton-shortened");

                    selfSizeButton.RemoveFromClassList("toolbarComboButton-normal");
                    selfSizeButton.AddToClassList("toolbarComboButton-shortened");
                }
            }
            ...
            if (width > 175)
            {
                ...
                rotationLabel.style.display = DisplayStyle.Flex;
                scaleLabel.style.display = DisplayStyle.Flex;

                if (sizeLabel != null)
                    sizeLabel.style.display = DisplayStyle.Flex;

                if (sizeCenterFoldoutField != null)
                    sizeCenterFoldoutField.label = "Center";
            }
            else
            {
                positionLabel.style.display = DisplayStyle.None;
                rotationLabel.style.display = DisplayStyle.None;
                scaleLabel.style.display = DisplayStyle.None;

                if (sizeLabel != null)
                    if (sizeLabel.style != null) //Note to self : I don't think style can be null, still there was a error reported due to it. Couldn't replicate the error but since this will run in edge case sceneries for 1 frame, one extra if statement even if unnecessary isn't that bad.
                        sizeLabel.style.display = DisplayStyle.None;

                if (sizeCenterFoldoutField != null)
                    sizeCenterFoldoutField.label = "";
            }
        }
